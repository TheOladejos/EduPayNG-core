...
export class PricingService {
  ...
  async getInstitutionPricing() {
    const { data } = await this.supabase.admin
      .from("institutions")
      .select(
        "id, code, name, short_name, token_price, vendor_cost, gateway, is_active",
      )
      .order("display_order");
    ...
    return (data ?? []).map((i) => ({
      ...
      marginPct:
        Number(i.vendor_cost) > 0
          ? Math.round(
              ((Number(i.token_price) - Number(i.vendor_cost)) /
                Number(i.vendor_cost)) *
                100,
            )
          : 0,
    }));
  ...
  async updateInstitutionPricing(
    ...
    await this.supabase.admin.from("audit_logs").insert({
      action: "INSTITUTION_PRICE_UPDATED",
      ...
      metadata: {
        ...
        changes: updates,
        before: {
          token_price: existing.token_price,
          vendor_cost: existing.vendor_cost,
        },
      },
    });

    this.logger.log(
      `Institution pricing updated: ${existing.code} → ₦${dto.sellingPrice ?? existing.token_price} by ${adminEmail}`,
    );
  ...
  async syncTokenFromVtpass(adminEmail: string): Promise<{
    ...
    const { data: institutions } = await this.supabase.admin
      .from("institutions")
      .select("*")
      .eq("is_active", true);
      ...
      try {
        ...
        for (const v of variations) {
          ...
          if (Number(institution.vendor_cost) !== Number(v.variationAmount)) {
            ...
            totalUpdated++;
            this.logger.log(
              `Price updated: ${institution.short_name} — ${v.name}: ₦${institution.vendor_cost} → ₦${v.variationAmount}`,
            );
          }
        }
      } catch (err: any) {
        const msg = `Failed to sync ${institution.short_name}: ${err.message}`;
    ...
    await this.supabase.admin.from("audit_logs").insert({
      action: "INSTITUTION_PRODUCTS_SYNCED",
      resource_type: "INSTITUTION_PRODUCTS",
      metadata: { by: adminEmail, priceUpdates: totalUpdated, errors },
    });
  ...
  async getBillProducts(billerId?: string) {
    let q = this.supabase.admin
      .from("bill_products")
      .select(
        "id, vtpass_code, name, description, amount, validity, is_active, billers(id, name, short_name, category_code)",
      )
  ...
  async syncBillProductsFromVtpass(adminEmail: string): Promise<{
    ...
    for (const biller of billers ?? []) {
      try {
        this.logger.log(
          `Syncing products for ${biller.short_name} (${biller.vtpass_code})`,
        );
        ...
        for (const v of variations) {
          ...
          if (existing) {
            ...
            if (Number(existing.amount) !== Number(v.variationAmount)) {
              await this.supabase.admin
                .from("bill_products")
                .update({ amount: Number(v.variationAmount), name: v.name })
                .eq("id", existing.id);
              totalUpdated++;
              this.logger.log(
                `Price updated: ${biller.short_name} — ${v.name}: ₦${existing.amount} → ₦${v.variationAmount}`,
              );
            }
          } else {
        ...
        await this.supabase.admin
          ...
          .not(
            "vtpass_code",
            "in",
            `(${activeVtpassCodes.map((c) => `"${c}"`).join(",")})`,
          );
      } catch (err: any) {
    ...
    await this.supabase.admin.from("audit_logs").insert({
      action: "BILL_PRODUCTS_SYNCED",
      resource_type: "BILL_PRODUCTS",
      metadata: {
        by: adminEmail,
        newProducts: totalSynced,
        priceUpdates: totalUpdated,
        errors,
      },
    });
      ...
      return [
        {
          ...
          margin_pct: 2,
          margin_flat: 0,
          note: "2% margin on airtime purchases",
        },
        {
          category_code: "DATA",
          margin_pct: 5,
          margin_flat: 0,
          note: "5% margin on data bundles",
        },
      ];
    }

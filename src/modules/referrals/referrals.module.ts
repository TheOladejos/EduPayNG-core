...
export class ReferralsService {
  ...
  async getReferrals(userId: string) {
    let { data: refInfo } = await this.supabase.admin
      .from("referral_codes")
      .select("code")
      .eq("user_id", userId)
      .maybeSingle();
    ...
    const { data: referrals } = await this.supabase.admin
      .from("referrals")
      .select(
        "id, status, reward_amount, reward_credited, created_at, completed_at",
      )
...
export class ReferralsController {
  ...
  @Get()
  @ApiOperation({ summary: "Get referral program details and history" })
  get(@CurrentUser() u: AuthUser) {
    return this.svc.getReferrals(u.id);
  }
}

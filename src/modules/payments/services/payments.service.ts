...
export class PaymentsService {
  ...
  async verifyTransaction(userId: string, reference: string) {
    const { data, error } = await this.supabase.admin
      .from("transactions")
      .select(
        "id, status, amount, payment_method, transaction_type, completed_at, created_at, metadata",
      )
  ...
  async handlePaystackWebhook(
    ...
    if (!this.paystack.verifyWebhookSignature(rawBody, signature)) {
      this.logger.warn("Paystack: Invalid signature — rejected");
      return;
    }
    ...
    if (type === "WALLET_FUNDING" || !type) {
      const { data: txn } = await this.supabase.admin
        ...
        .maybeSingle();
      if (!txn) {
        this.logger.warn(`Paystack: ref not found ${reference}`);
        return;
      }
      if (txn.status !== "PENDING") {
        this.logger.log(`Paystack: already processed ${reference}`);
        return;
      }
      ...
      await this.wallet.creditWallet(
        txn.user_id,
        amountNaira,
        `Wallet funded via Paystack (${reference})`,
      );
      await this.wallet.sendNotification(
        txn.user_id,
        "💰 Wallet Funded",
        `₦${amountNaira.toLocaleString()} added to your wallet.`,
        "SUCCESS",
        "TRANSACTION",
      );
      return;
    ...
    if (type === "TOKEN_PURCHASE") {
      const { data: txn } = await this.supabase.admin
        ...
        .maybeSingle();
      if (!txn) {
        this.logger.warn(`Token ref not found: ${reference}`);
        return;
      }
      if (txn.status !== "PENDING") {
        this.logger.log(`Already processed: ${reference}`);
        return;
      }
      ...
      const { institutionId, gateway, quantity, deliveryMethod, vendorCost } =
        paystackMeta ?? {};
    ...
    if (type === "HYBRID_TOKEN_PURCHASE") {
      ...
      this.logger.log(
        `Hybrid payment confirmed: ${reference} (card ₦${amountNaira}) for mainRef ${mainRef}`,
      );
      ...
      const { data: hold } = await this.supabase.admin
        .from("wallet_holds")
        .select("*")
        .eq("transaction_ref", mainRef)
        .eq("status", "HOLDING")
        .single();
      if (!hold) {
        this.logger.warn(`Hold not found for mainRef: ${mainRef}`);
        return;
      }
    ...
    if (type === "POINT_PURCHASE") {
      ...
      if (pointsAwarded) {
        ...
        if (w)
          await this.supabase.admin
            .from("wallets")
            .update({ points: w.points + pointsAwarded })
            .eq("user_id", txn.user_id);
      }
      await this.wallet.sendNotification(
        txn.user_id,
        "⭐ Points Added",
        `${(paystackMeta?.pointsAwarded ?? 0).toLocaleString()} points added.`,
        "SUCCESS",
        "TRANSACTION",
      );
    }
  ...
  async handlePaystackChargeFailure(
    rawBody: Buffer,
    signature: string,
  ): Promise<void> {
  ...
  async handleVtpassWebhook(payload: any): Promise<void> {
    ...
    const isDelivered =
      payload.code === "000" || payload.status === "delivered";
    ...
    if (isDelivered) {
      await this.supabase.admin
        ...
        .update({
          status: "SUCCESS",
          vtpass_status: "delivered",
          vtpass_response: payload,
      ...
      await this.wallet.creditWallet(
        billTxn.user_id,
        billTxn.amount,
        `Refund: ${billTxn.category_code} failed`,
      );
      await this.supabase.admin
        .from("bill_transactions")
        .update({
          status: "FAILED",
          vtpass_status: "failed",
          failure_reason: payload.response_description,
          updated_at: new Date().toISOString(),
        })
        .eq("id", billTxn.id);
      await this.supabase.admin
      ...
      await this.wallet.sendNotification(
        billTxn.user_id,
        "❌ Bill Failed",
        `Your bill failed. ₦${Number(billTxn.amount).toLocaleString()} has been refunded.`,
        "ERROR",
        "BILL",
      );
    }
